...
3. Extension ID가 OAuth consent에 등록된 값과 일치하는지 확인 (`WXT_EXTENSION_PUBLIC_KEY`로부터 결정론적으로 파생)
...
1. `.env`에 PoC용 `WXT_EXTENSION_PUBLIC_KEY` 설정 → 해당 키로부터 Extension ID가 고정됨
2. 해당 Extension ID가 Google Cloud Console OAuth client의 애플리케이션 ID 필드에 등록되어 있는지 확인 (`WXT_OAUTH_CLIENT_ID`와 동일한 클라이언트)
3. `pnpm zip` → `.output/event-profiler-0.1.0-chrome.zip` 생성
4. 사내 공지 채널(Slack/Drive 등)에 zip + 아래 설치 가이드 공유

**설치자(비개발자)**

1. zip 파일 다운로드 → 삭제되지 않을 고정 경로에 **압축 해제** (예: `~/Tools/event-profiler/`)
