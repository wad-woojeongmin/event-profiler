...
// Pass·Missing은 progress bar로 전체 대비 비율도 보여 준다.
...
export function StatsDashboard({ stats }: Props) {
  ...
  const missingPct = Math.round((stats.notCollected / total) * 100);
  return (
    <section className={styles.grid} aria-label="요약 통계">
      <BigStat kind="pass" label="Pass" value={stats.pass} pct={passPct} />
      <BigStat kind="fail" label="Fail" value={stats.fail} />
      <BigStat kind="warn" label="중복 의심" value={stats.suspectDuplicate} />
      <BigStat
        kind="missing"
        label="미수집"
