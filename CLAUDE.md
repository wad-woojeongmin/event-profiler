...
본 프로젝트는 [WXT](https://wxt.dev) 기반 웹 확장 프로그램입니다.

## 요구사항 문서 구조

- `REQUIREMENTS.md` — 프로젝트 최상위 엔트리 (짧은 인덱스)
- `docs/00-index.md` — 에이전트 라우팅·문서 맵 (작업 시작 지점)
- `docs/01-overview.md` ~ `docs/05-sheet-spec.md` — 공통 컨텍스트
- `docs/modules/m1~m8.md` — 모듈별 요구사항

**작업 착수 순서**: `REQUIREMENTS.md` → `docs/00-index.md` → 자신이 맡은 모듈 문서 + 해당 모듈의 "필독" 링크만 읽는다. 전체 문서를 매번 훑지 않는다.
...
4. 프로젝트 고유 요구사항(시트 스키마, 메시지 프로토콜, 모듈 분담 등)은 `docs/` 하위 문서가 우선이며, WXT 문서와 충돌하면 `docs/`를 따른다.
