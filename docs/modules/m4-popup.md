...
- 하단: 녹화 제어
  ...
  - 녹화 완료:
    - **Phase 1**: `[리포트 생성 (새 탭)]` + `[다시 녹화]` (선택 초기화 없이 재시작)
    - **Phase 2**: `[HTML 다운로드]`(M8 산출물 의존), `[초기화]`(세션/선택 리셋)
...
- [ ] 접근성: 키보드 네비게이션 지원
- [ ] `[다시 녹화]`로 동일 선택으로 재시작 가능

### Phase 2 (후속)

- [ ] `[HTML 다운로드]` — M8의 HTML 리포트 산출물이 준비되면 추가
- [ ] `[초기화]` — 세션/선택 상태를 명시적으로 리셋하는 버튼
- [ ] 파서 경고 배지 — `ParseResult.warnings`를 팝업 UI에 노출
- [ ] 대량 스펙 가상화 — 1k+ 이벤트 시트 대응
